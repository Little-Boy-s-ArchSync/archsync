...
[ADR-0004](docs/adr/0004-architecture-change-policy-proposed.md) contains the GOV-103 risk/approval matrix and fail-closed baseline-update policy. Its reviewed bytes retain the **Proposed** label by design; the separate [human approval evidence](docs/adr/acceptance-evidence/GOV-103-r1.json) and [append-only closure record](docs/adr/acceptance-records/20260911-GOV-103-r1-a65edb493b48.json) are now present on protected `main`. `pnpm governance:verify` checks their schema, exact-byte bindings and P < E < C history, reporting `CLOSURE_RECORD_VALIDATED_AUTHENTICITY_REVIEW_REQUIRED`; local validation does not independently establish human authenticity or authorize a future architecture change.
